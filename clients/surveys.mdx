...
Survey references let you embed one survey inside another. The respondent sees the referenced survey's blocks inline — they don't know they've crossed into a different survey. This is primarily used to reuse a standard set of questions (like profiling questions) across multiple surveys without duplicating them.

### How References Work

When a respondent reaches a [Survey Reference](#survey-reference) block, the referenced survey's blocks are presented inline within the parent survey. The respondent progresses through them as if they were part of the parent survey. When the referenced survey's blocks are complete, the respondent continues with the next block in the parent survey.

Responses are stored on the **referenced** survey's questions, not the parent's. This is the key benefit for reuse — profiling data updates on the same questions regardless of which survey referenced it.

### When References Are Skipped

A survey reference is skipped — the respondent moves past it automatically — when:

- The referenced survey isn't published
- The respondent has already completed the referenced survey (and it doesn't allow multiple submissions)
- The respondent doesn't meet the referenced survey's prerequisites

This means if a panelist has already taken a referenced profiling survey through another distribution or another parent survey, they won't be asked those questions again.

### Eligibility and Disqualification

Eligibility is tracked independently for the parent survey and each referenced survey. If a respondent is disqualified inside a referenced survey (via a Disqualify block, choice eligibility, etc.), that disqualification applies only to the referenced survey's respondent record — it does **not** carry back to the parent survey.

"End Survey When Disqualified" inside a referenced survey routes the respondent to the referenced survey's ending, which then returns them to the parent survey to continue.

### Payment

Gratuity is calculated and paid once for the **parent** survey only — referenced surveys don't trigger separate payments. However, the time spent answering questions in referenced surveys **is included** in the parent survey's gratuity calculation. The panelist receives a single payment that accounts for the total time across all surveys in the chain.

<Tip>
Create your profiling questions as their own survey, then reference it from each research survey using a Survey Reference block. Responses update the same question data regardless of which parent survey referenced it — and panelists who've already completed the profiling survey skip it automatically.
</Tip>
...
The survey dashboard is the overview you see when you open a survey. It provides at-a-glance metrics so you can monitor how your survey is performing and track your spend as responses come in.

The dashboard has two sections: a top row of summary stats and a qualification breakdown below it.

## Summary Stats

- **Sent** — The total number of panelists who have been sent the survey across all distributions.
- **Completions** — The number of completed responses received, along with the date of the most recent completion so you can see how recently activity has come in.
- **Response Rate** — The percentage of panelists who completed the survey out of those who were sent it (completions / sent). This is your primary signal for how well your survey is performing — a low response rate may indicate issues with your targeting, survey length, or subject matter.
- **Gratuity** — The total dollar amount paid for respondents for this survey. This lets you monitor your spend in real time as responses come in, rather than waiting until the survey is complete to reconcile costs. Since gratuity is calculated [per question](/clients/surveys#survey-gratuity) based on how far each respondent progresses, this number updates as each respondent finishes.

## Qualification Breakdown

Below the summary stats, a progress bar visualizes how your completions break down by eligibility status:

- **Qualified** — Respondents who completed the survey and met your eligibility criteria.
- **Flex** — Respondents flagged as "maybe" via Flex blocks, awaiting manual review.
- **Disqualified** — Respondents who were disqualified based on their answers or action blocks.

Each status shows both a count and a percentage of total responses. If your disqualified count is running high relative to your qualified count, you may want to revisit your targeting or distribution audience.
