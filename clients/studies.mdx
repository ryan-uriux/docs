...
Sometimes you need to compensate a participant for additional work after their original session — a follow-up interview, a diary study entry, or a product trial. To pay someone for a follow-up, create a session for the time the follow-up occurred and mark it as invite-only. Then go to the `Candidates` tab, click the `...` menu on the participant's row, and select `Manually Schedule` to schedule them into the session. They'll be paid their standard hourly rate for that session's duration.
